...
=cut
    ...
    while (1) {
        ...
        randomgraph3($self, $n, $g) if ($method == 3);
        randomgraph4($self, $n, $g) if ($method == 4);
...
=head1 randomgraph3
    ...
    Private method. Arguments: (graph object, number of vertices, degree of vertices)
    Uses the algorithm described in Wikipedia to generate a random r-regular graph:
...
=cut
    ...
    ATTEMPT: while(1) {
        my (@pre_mat);
        push @pre_mat, [(0) x $n] for (1..$n);
    
        my @pre_vertices = shuffle (0..$rn-1);
        
        MAKE_EDGES: for (0 .. $rn / 2 - 1) { 
            my $v1 = floor($pre_vertices[2*$_]/$r);
            my $v2 = floor($pre_vertices[2*$_ + 1]/$r);
        
            #disallow loops
            next ATTEMPT if ($v1 == $v2);
            
            #disallow multiedges
            next ATTEMPT if ( $pre_mat[$v1][$v2]);
            
            #if we are fine, create edge
            $pre_mat[$v1][$v2] = 1;
            $pre_mat[$v2][$v1] = 1;
        }
...
=head1 randomgraph4
    ...
    Private method. Arguments: (graph object, number of vertices, genus)
    Make a random degree sequence and then pass it to randomgraph_degree_seq
    
    So each vertex has degree at least 2. If we substract 2 from the degree of each vertex, then the sum of what remains is 2g - 2.
    So we need n random numbers that add up to 2g-2. We produce n-1 random integers between 0 and 2g - 2 and then we take the differences 
    to obtain our n random numbers. See:
    http://stackoverflow.com/a/8068956
=cut

    my $self = shift();
    my $n  = shift;
    my $g =  shift;
    my (@mat, @rand_numbers, @deg_seq);
    
    push @rand_numbers, entropy_source->get_int(2*$g - 1)  for (1 .. $n - 1);
    @rand_numbers = sort {$a <=> $b} @rand_numbers;
    
    #add a zero and 2*$g - 2 at the beginning/end.
    push @rand_numbers, 2*$g - 2;
    unshift @rand_numbers, 0;
    
    #make deg sequence, have everyone have deg at least 2
    push @deg_seq, 2 + $rand_numbers[$_] - $rand_numbers[$_ - 1] for (1.. $#rand_numbers);
    

    $self->{'matrix'} = randomgraph_degree_seq($n, $g, \@deg_seq, 0);
...
sub randomgraph_degree_seq { 
=head1 randomgraph_degree_seq

    Private method. Arguments: (graph object, number of vertices, genus)
    Based in the algorithm described in Wikipedia to generate a random r-regular graph:
    https://en.wikipedia.org/wiki/Random_regular_graph
    
    Makes the bins and the random pairing, but then groups everything according to the given degree seq      
=cut
    my $n  = shift;
    my $g =  shift;
    my @degs = @{shift()};
    my $disallow_multi_edges = shift;
    my $e = ($g + $n - 1)*2;
    my (@mat, @vert);
    
    #tell me where each edge goes
    my $i = 0;
    @vert = map {($i++) x $_} @degs;
    ...
    ATTEMPT: while(1) {
        ...
        my @pre_vertices = shuffle (0..$e-1);
        
        MAKE_EDGES: for (0 .. $e / 2 - 1) { 
            my $v1 = $vert[$pre_vertices[2*$_]];
            my $v2 = $vert[$pre_vertices[2*$_ + 1]];
        
            #disallow loops
            next ATTEMPT if ($v1 == $v2);
            
            #disallow multiedges
            next ATTEMPT if ($disallow_multi_edges && $pre_mat[$v1][$v2]);
...
sub all_rank1_energy {
    ...
    while (1) {
        ...
        for my $i (0 .. $n - 1) {
            if ($div[$i] > 0) {
                #$result[$_] += $div[$i] *$Li[$i][$_] foreach (0.. $n - 1);
                $result[$_] += ($div[$i] *$Li[$i][$_])  foreach (0.. $n - 1);
            }
        }
        #$result[$_] = $result[$_] - floor($result[$_]) foreach (0.. $n - 1);
        $result[$_] = $result[$_] % $det foreach (0..$n-1);
        ...
        push @reps, \@result if (!defined($R1{$result}));
        
        $R1{$result} = \@div;
        #$R2{array2div(@div)} = 1 if (!_burn(0, \@div));
    ...
    #say _matrix2string(@Li);
    
    #Do the intersections
    
    for my $i (1..$n-1) #Insert a rank determining set here
    {
        my %intersection;
        
        foreach my $class (@reps) {
            my @class = @$class;
            my @result;
            $result[$_] = ($class[$_] + $Li[$i][$_]) % $det foreach (0..$n-1);
            my $string = join ",", @result;
            $intersection{$string} = $R1{$string} if (defined($R1{$string}));
        }
        %R1 = %intersection;
    }
    
    #say scalar keys %R1, "\t", tv_interval($t0);
...
sub divisor_energy {
=head1 divisor_energy
returns a string with the Li*Di
Requires context
=cut
...
sub calculate_Li {
    ...
    $mat = $mat->inverse();
    my @Li;
    $Li[0][$_] = 0 foreach (0 .. $self->{n}-1);
    
    foreach my $i (1..$self->{n} -1) {
        $Li[$i][0] = 0;
        $Li[$i][$_]  = round($mat->element($i,$_)) foreach (1..$self->{n}-1);
        #$Li[$i][$_]  = $mat->element($i,$_) foreach (1..$self->{n}-1);
    }
    #say "======";
    #say _matrix2string(@Li);
    #exit;
=test
    ...
    say "Now a lap";
    say _matrix2string(@{$self->{lap}});
    
    
    say "Now a product"; 
    my @Li2;

    $mat = Math::MatrixReal->new_from_rows($self->{lap}) * (Math::MatrixReal->new_from_rows(\@Li) / $det) *  Math::MatrixReal->new_from_rows($self->{lap});
    foreach my $i (1..$self->{n}) {
        $Li2[$i-1][$_-1]  = round($mat->element($i,$_)) foreach (1..$self->{n});
        #$Li[$i][$_]  = $mat->element($i,$_) foreach (1..$self->{n}-1);
    }
    say _matrix2string(@Li2);
    exit;
=cut
...
sub _links2matrix_canonical {
    ...
    #Yo dawg, I heard you like maps, so I put a map in your map in order to make names canonical.
    map { 
            map {$canonical{$_} = $k++ unless (defined($canonical{$_}))} split ',', $_; 
        } 
        split ' ' foreach (@_);
    
    #convert to chain, this is a clever contraption
    map { ($i, $j) = map {$canonical{$_}} split ',', $_; 
        $mat[$i][$j]++;
        $mat[$j][$i]++;
        $n = $j if ($n < $j);
        $n = $i if ($n < $i);} 
...
=cut
    ...
    #print "1", "\t", scalar @divisors, "\t", tv_interval($t0), "\t";
...
sub _all_divisors_iterator3 {
=pod
    We receive $c, the number of stones to use
    We store energy strings in a hash
=cut
